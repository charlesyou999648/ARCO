...
class Decoder(nn.Module):
    ...
    def __init__(self, params):
        ...
        self.nonLinearPro = torch.nn.Sequential(
            torch.nn.Conv2d(self.ft_chns[0], self.ft_chns[0], (1, 1)),
            torch.nn.ReLU(),
            torch.nn.Conv2d(self.ft_chns[0], self.n_class, (1, 1)))
    ...
    def __init__(self, in_chns, class_num, train_encoder=True, train_decoder=True, unfreeze_seg=True):
        ...
        if not (unfreeze_seg):
            ...
            for params in self.decoder.parameters():
                params.requires_grad = False
                params = params.detach_()
            for params in self.decoder.out_conv.parameters():
                params.requires_grad = True
            for params in self.decoder.nonLinearPro.parameters():
                params.requires_grad = True
                # if params not in self.decoder.out_conv.parameters() and \
                #     params not in self.decoder.nonLinearPro.parameters():
