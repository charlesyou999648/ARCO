...
class RandTPS(nn.Module):
    def __init__(self, width, height, depth, batch_size=16, sigma=0.01, border_padding=False, random_mirror=True, random_scale=(0.7,1.1), mode='affine'):
    ...
    def forward(self, x, padding_mode=None, mode='bilinear'):
        ...
        if len(x.shape) == 4:
            ...
            for d in range(x.shape[-1]):
                img = x[..., d]
                if padding_mode is None:
                    img = grid_sample(img, self.grid, canvas=None, padding_mode=self.padding_mode, mode=mode)
                else:
                    img = grid_sample(img, self.grid, canvas=None, padding_mode=padding_mode, mode=mode)
