...
class LAHeartWithIndex(Dataset):
    """ LA Dataset """
    def __init__(self, base_dir=None, split='train', num=None, transform=None, 
                 index=4, label_type=1):
        self._base_dir = base_dir
        self.transform = transform
        self.sample_list = []
        if split=='train':
        ...
        if np.random.uniform(low=0, high=1, size=1) > self.p:
            ...
            for i in range(image.shape[0]):
                for j in range(image.shape[-1]):
                    image[i, 0, :, :, j] = torch.FloatTensor(np.array(ToPILImage()(image[i, 0, :, :, j]).filter(ImageFilter.GaussianBlur(radius=sigma))))
...
if __name__ == '__main__':
    ...
    train_data_path = '/home/weicheng/selfLearning/DTC/data/2018LA_Seg_Training Set'
